...
public enum Network_ProcessRole: Int, Codable {
    case application = 0
    case proxyOrVPN = 1
    case tunnelInfrastructure = 2
}
...
public struct Network_Process: Codable, Process_p {
    ...
    public var role: Network_ProcessRole
    ...
    public var icon: NSImage {
        get {
            ...
            if let path = self.appPath, let icon = Network_Process.bundleIcon(fromPath: path) {
                return icon
            }
    ...
    // NSWorkspace.icon(forFile:) is unreliable (returns a shared placeholder
    // for real apps, especially off the main thread) — load the app's .icns
    // directly from its bundle instead: a plain file read that works on any
    // thread.
    private static func bundleIcon(fromPath appPath: String) -> NSImage? {
        let fm = FileManager.default
        let resources = appPath + "/Contents/Resources"
        var candidates: [URL] = []
        if let info = NSDictionary(contentsOfFile: appPath + "/Contents/Info.plist") {
            if let file = (info["CFBundleIconFile"] as? String) ?? (info["CFBundleIconName"] as? String), !file.isEmpty {
                let name = file.hasSuffix(".icns") ? file : file + ".icns"
                let url = URL(fileURLWithPath: resources).appendingPathComponent(name)
                if fm.fileExists(atPath: url.path) { candidates.append(url) }
            }
        }
        if candidates.isEmpty, let items = try? fm.contentsOfDirectory(atPath: resources) {
            let icns = items.filter{ $0.hasSuffix(".icns") }
                .map{ URL(fileURLWithPath: resources).appendingPathComponent($0) }
                .sorted { ((try? fm.attributesOfItem(atPath: $1.path))?[.size] as? Int ?? 0) < ((try? fm.attributesOfItem(atPath: $0.path))?[.size] as? Int ?? 0) }
            candidates = [icns.first].compactMap{ $0 }
        }
    ...
    public var badge: String? {
        get {
            if self.role == .proxyOrVPN { return "VPN" }
            return nil
        }
    }
    
    public init(pid: Int = 0, name: String = "", time: Date = Date(), download: Int = 0, upload: Int = 0, isSystem: Bool = false, count: Int = 1, role: Network_ProcessRole = .application, appPath: String? = nil) {
        self.pid = pid
        self.name = name
        self.time = time
