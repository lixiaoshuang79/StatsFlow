...
public func popupInlineBadgeRow(_ view: NSView? = nil, title: String, status: Bool? = nil, ok: String = "UP", notOk: String = "DOWN") -> (LabelField, StatusBadgeView, NSView) {
    let width = view?.frame.width ?? 0
    let height: CGFloat = 22
    let rowView: NSView = NSView(frame: NSRect(x: 0, y: 0, width: width, height: height))
    
    let labelWidth = title.widthOfString(usingFont: .systemFont(ofSize: 12, weight: .regular)) + 6
    let labelView: LabelField = LabelField(frame: NSRect(x: isRTL ? width - labelWidth : 0, y: (height-16)/2, width: labelWidth, height: 16), title)
    
    let badgeView = StatusBadgeView(frame: NSRect(x: isRTL ? 0 : labelWidth, y: (height-14)/2, width: 50, height: 14), status, ok: ok, notOk: notOk)
...
public func popupInlineRow(_ view: NSView? = nil, title: String, value: String, multiline: Bool = false) -> (LabelField, ValueField, NSView) {
    ...
    let valueView: ValueField = ValueField(frame: NSRect(x: isRTL ? 0 : labelWidth, y: (22-16)/2, width: max(10, rowView.frame.width - labelWidth), height: multiline ? 16*lines : 16), value)
